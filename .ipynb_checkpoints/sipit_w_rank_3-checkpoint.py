...
def sipit(
    ...
    for t in range(seq_len):
        ...
        # NOWE: Zmienne do śledzenia wczesnego zatrzymania i odrzuconych tokenów
        banned_tokens = set()
        found_token = None
        found_rank = -1  # -1 oznacza, że znaleziono go w locie (early stop)
        ...
        for step in range(steps):
            ...
            # NOWE: Okresowe rzutowanie na twardy token
            if step > 0 and step % discrete_check_interval == 0:
                with torch.no_grad():
                    ...
                    if hard_error < loss_th:
                        # Znaleziono dobry token, przerywamy całą optymalizację
        ...
        # Jeśli nie znaleźliśmy tokena w trakcie lotu (early stop dyskretny), szukamy standardowo
        if found_token is None:
            with torch.no_grad():
                optimized_vec = proxy_emb.detach().squeeze()
                dists = compute_distances(embedding_matrix, optimized_vec, metric=metric)
                
                # Upewniamy się, że odrzucone tokeny nie wrócą w finalnym rankingu
                if banned_tokens:
                    dists[list(banned_tokens)] = float('inf')
            ...
            for rank, cand_id in enumerate(candidates):
                ...
                if error < loss_th:
                    ...
                    banned_tokens.add(cand_id) # Opcjonalne, ale spójne
        ...
        token_stats = {
            ...
            "banned_tokens_count": len(banned_tokens) # Dodatkowa statystyka
        }
