...
def compute_distances(
    ...
    if metric == 'l2':
        if emb_norms_sq is not None:
            # ||e - v||_2 = sqrt(||e||^2 + ||v||^2 - 2 e·v); same argmin as squared form
            dots = matrix.mv(vector)
            d2 = emb_norms_sq - 2.0 * dots + vector.dot(vector)
            return torch.sqrt(d2.clamp(min=0.0))
    ...
    for t in range(seq_len):
            ...
            if step > 0 and step % discrete_check_interval == 0:
                with torch.no_grad():
                    ...
                    dists = compute_distances(
                        embedding_matrix, current_vec, metric=metric, emb_norms_sq=emb_norms_sq
                    )
        ...
        if found_token is None:
            with torch.no_grad():
                ...
                dists = compute_distances(
                    embedding_matrix, optimized_vec, metric=metric, emb_norms_sq=emb_norms_sq
                )
                
                if banned_tokens:
                    dists[list(banned_tokens)] = float('inf')
                    
                candidates = torch.argsort(dists)[:1].tolist()
        ...
        with torch.no_grad():
            inp_commit = torch.tensor([[found_token]], device=device)
            out_commit = model(
                input_ids=inp_commit,
                past_key_values=past_key_values,
                use_cache=True,
                output_hidden_states=True,
            )
            h_discrete = out_commit.hidden_states[-1][0, -1, :]
            discrete_loss = F.mse_loss(h_discrete, target_h).item()
            past_key_values = detach_past(out_commit.past_key_values)
